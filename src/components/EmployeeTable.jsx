...
export default function EmployeeTable({ employees, onDelete, onEdit, onSort, sortConfig }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
      <div className="overflow-x-auto">
        {/* 'table-fixed' zwingt den Browser, sich strikt an die th-Breiten zu halten */}
        <table className="w-full text-left border-collapse table-fixed min-w-[640px]" aria-label="Mitarbeiterliste">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200 text-xs font-bold uppercase text-slate-500">             
              ...
              <th className="w-[140px] py-3 px-4 text-right">
                {t('actions')}
              </th>
            </tr>
          </thead>
              ...
              employees.map((emp) => (
                <tr key={emp.id} className="hover:bg-slate-50/80 transition"> 
                  ...
                  <td className="py-3 px-4 text-right space-x-1 whitespace-nowrap">
                    ...
                    <button  onClick={() => onEdit(emp)} aria-label={t('edit')}
                      className="inline-flex items-center justify-center p-1.5 bg-amber-50 hover:bg-amber-100 text-amber-700 rounded-lg border border-amber-200 transition focus:ring-2 focus:ring-amber-500 focus:outline-none cursor-pointer group/btn relative"
                    >
                      <PencilSquareIcon className="w-4 h-4" />                    
                      <span className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 hidden group-hover/btn:block bg-slate-800 text-white text-[10px] font-bold px-1.5 py-0.5 rounded shadow-xs z-20 pointer-events-none">
                        {t('edit')}
                      </span>
                    </button>                  
                    <button onClick={() => onDelete(emp.id)} aria-label={t('delete')}
                      className="inline-flex items-center justify-center p-1.5 bg-red-50 hover:bg-red-100 text-red-700 rounded-lg border border-red-200 transition focus:ring-2 focus:ring-red-500 focus:outline-none cursor-pointer group/btn relative"
                    >
